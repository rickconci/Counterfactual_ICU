import wandb
from utils_4 import interpolate_colors
import numpy as np
...
def plot_SDE_trajectories(X, Y, Y_hat, Y_cf, Y_hat_cf, latent_traj, chart_type="val"):
...
def plot_trajectories(X, Y, Y_hat, latent_traj, chart_type="val"):
    # Ensure tensors are on CPU for plotting
    X = X.cpu()
    Y = Y.cpu()
    Y_hat = Y_hat.cpu()
