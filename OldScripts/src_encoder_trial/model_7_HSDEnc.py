...
class Hybrid_VAE_SDE_Encoder(LightningModule):
    ...
    def __init__(self, expert_latent_dims, path_control_dim, apply_path_SDE, 
        ...
        self.py0_mean = nn.Parameter(mu_tensor, requires_grad=False)
        self.py0_logvar = nn.Parameter(logvar_tensor, requires_grad=False)
    ...
    def f(self, t, y):  # Approximate posterior drift.
        ...
        print('ca, cv', ca[0].item(), cv[0].item())
        print('dpa_dt, dpv_dt', dpa_dt[0].item(),dpv_dt[0].item() )
        print('time,f_hr,r_tpr, dva_dt, dvv_dt, dpa_dt, dpv_dt, ds_dt, dsv_dt', t.item(),f_hr[0].item(), r_tpr[0].item(), dva_dt[0].item(), dvv_dt[0].item(),ds_dt[0].item() , dsv_dt[0].item())   
    ...
    def training_step(self, batch, batch_idx):
        ...
        print(z0.device, z1_input.device, logqp0.device, latent_traj.device,logqp_path.device, path_control.device, predicted_traj.device,loss.device, fact_loss.device, kl_loss.device,L1_loss.device  )
    ...
    def configure_optimizers(self):
        ...
        for param_group in optimizer.param_groups:
            for param in param_group['params']:
                print("Optimizer parameter device:", param.device)
        
        scheduler = {
            "monitor": "train_total_loss",
            "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, mode="min", factor=0.5, patience=50)
        }
    ...
    def visualize_distributions(self, num_samples=10000):
        ...
        with torch.no_grad():
            ...
            mean_q = self.qy0_mean.cpu().numpy()
            std_q = torch.clamp(self.qy0_std, min=1e-6).cpu().numpy()
            mean_p = self.py0_mean.cpu().numpy()
            std_p = torch.clamp(self.py0_std, min=1e-6).cpu().numpy()
        ...
        with torch.no_grad():
            for i, b in enumerate(idx):
                for d in range(num_dims):
                    pred_samples = predicted_traj[b, :, :, d].cpu().numpy()
                    true_values = true_traj[b, :, d].cpu().numpy()
