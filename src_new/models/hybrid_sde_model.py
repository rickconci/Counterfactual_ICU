...
class Hybrid_SDE(LightningModule):
    ...
    def __init__(self, use_encoder, start_dec_at_treatment, variational_sampling, 
        ...
        if self.debug: print(f"[DEBUG] Hybrid_SDE __init__: Initializing... adjoint={adjoint}, use_encoder={use_encoder}, normalise_for_SDENN={normalise_for_SDENN}")
        ...
        if self.debug: print(f"[DEBUG] Hybrid_SDE __init__: Initialization complete. Encoder output dim: {self.encoder_output_dim}, SDEnet input dims: {net_input_dims}")
        ...
        if self.use_encoder == 'raindrop':
            ...
            if self.start_dec_at_treatment:
                if self.variational_encoder:
                    ...
                    if self.debug: print(f"[DEBUG] Hybrid_SDE forward_enc (non-variational): Encoder output z1_shape (before repeat): {z1.shape}")
            ...
            z1 = input_vals.unsqueeze(1).repeat(1, self.num_samples, 1)
            if self.debug: print(f"[DEBUG] Hybrid_SDE forward_enc (no encoder, no variational sampling): z1_shape={z1.shape}")
            logqp0 = 0
            z1_logvar = None


        if self.debug: print(f"[DEBUG] Hybrid_SDE forward_enc: Returning z1_shape={z1.shape}, z1_logvar_type={type(z1_logvar)}, logqp0_type={type(logqp0)}")
    ...
    def h(self, t, y):  # Prior drift.
        if self.debug and t.item() % 10 == 0:
            print(f"[DEBUG] Hybrid_SDE h (prior drift): t={t.item()}, y_shape={y.shape}")
    ...
    def forward_dec(self, latent_out):
        ...
        """
        if self.debug: 
            print(f"[DEBUG] Hybrid_SDE forward_dec: latent_out_shape={latent_out.shape}")
        ...
        if self.debug: print(f"[DEBUG] Hybrid_SDE forward_dec: decoded_mean_shape={output_traj.shape}")
        return output_traj
    ...
    def compute_factual_loss(self, predicted_traj, true_traj, logqp, mask=None):
        ...
        if self.debug:
            ...
            print(f"[DEBUG] Hybrid_SDE compute_factual_loss: Shapes: predicted_traj={predicted_traj.shape}, true_traj={true_traj.shape}")
    ...
    def compute_counterfactual_loss(self, true_fact, true_cf, pred_fact, pred_cf):
        if self.debug: print(f"[DEBUG] Hybrid_SDE compute_counterfactual_loss: true_fact_shape={true_fact.shape}, pred_cf_shape={pred_cf.shape}")
        ...
        if self.debug: print(f"[DEBUG] Hybrid_SDE compute_counterfactual_loss: mse_fact={mse_cf.item()}, mse_cf={mse_cf.item()}")
