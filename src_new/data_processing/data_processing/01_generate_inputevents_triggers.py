...
from config import MIMIC_DATA_DIR, treatment_cols_to_keep, PROCESSED_DATA_DIR, HELPFUL_DF_DIR
...
def build_inputevents_mv_df(items_df: pd.DataFrame, hadm_ids: list[int]) -> pd.DataFrame:
    """
    Clean IV/medication input events and return a tidy dataframe with:
       subject_id, hadm_id, item_id, input_name, input_class,
       start_time, end_time, rate, rate_uom, rate/weight
...
def build_inputevents_cv_df(df: pd.DataFrame, items_df: pd.DataFrame, hadm_ids: list[int]) -> pd.DataFrame:
    """
    Clean IV/medication input events and return a tidy dataframe with:
       subject_id, hadm_id, item_id, input_name, input_class,
       start_time, end_time, rate, rate_uom, rate/weight
...
def _resolve_encounter_keys(df: pd.DataFrame):
    ...
    if "hadm_id" in df.columns:
        keys.append("hadm_id")
    if not keys:
        raise KeyError("Need at least one of 'subject_id' or 'hadm_id' (in columns or index).")
    return keys
...
def _resolve_med_key(df: pd.DataFrame):
    ...
    for k in ("item_label", "input_name", "item_id"):
        if k in df.columns:
            return k
    raise KeyError("Need one of 'item_label', 'input_name', or 'item_id' to group by medication.")
...
def compute_triggers(
    ...
    df0["rate_norm"] = df0["rate_weight_norm"].where(~df0["rate_weight_norm"].isna(), df0["rate"])
    ...
    if "input_class" in df0.columns:
        is_bolus = df0["input_class"].astype(str).str.contains("bolus|push", case=False, na=False)
    ...
    df0["prev_rate"] = df0.groupby(enc_keys + [med_key], group_keys=False)["rate_norm"].shift(1)
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        rel_delta = (abs_delta / df0["prev_rate"]).replace([np.inf, -np.inf], np.nan).fillna(1.0)

    significant_increase = (delta > 0) & (rel_delta >= relative_change) & (abs_delta >= abs_min_change)
    significant_decrease = (delta < 0) & (rel_delta >= relative_change) & (abs_delta >= abs_min_change)
    ...
    nacl_aliases = {
        "nacl 0.9%", "ns", "normal saline", "sodium chloride 0.9%", "0.9% sodium chloride"
    }
    ...
    if "input_name" in df0.columns:
        is_fluid_bolus = df0["input_name"].astype(str).str.contains("bolus", case=False, na=False)
    else:
        is_fluid_bolus = pd.Series(False, index=df0.index)

    base_trigger = first_admin | significant_increase | significant_decrease | is_fluid_bolus
    ...
    if throttle:
        def throttle_group(g):
            ...
            roll = (
                g2.set_index("start_time")["trigger"].astype(int)
                  .rolling(f"{window_minutes}min").sum()
            )
        ...
        throttled = df0.groupby(enc_keys + [med_key], group_keys=False).apply(throttle_group)
        throttled = throttled.astype(bool).reindex(df0.index)
        df0["trigger_reason"] = np.where(df0["trigger"] & (~throttled), "throttled_rolling_window", df0["trigger_reason"])
...
def main(save_dir = None):
    ...
    hadm_ids = pd.read_csv(HELPFUL_DF_DIR/'relevant_patient_ids.csv')['hadm_id'].tolist()
    inputevents_mv_df= build_inputevents_mv_df(items_df, hadm_ids)
    inputevents_mv_df_reset = inputevents_mv_df.copy().reset_index(drop=True)
    input_events_filtered_trigger_meds = compute_triggers(inputevents_mv_df_reset)
    res = compute_triggers(input_events_filtered_trigger_meds, throttle=False)
    if save_dir is not None:
        res.to_csv(save_dir/'input_events_filtered_trigger_meds_res.csv', index=False)
    ...
    if save_dir is not None:
        per_hadm.to_csv(save_dir/'input_events_filtered_trigger_meds_res_per_hadm.csv', index=False)
...
def compute_action_clusters(
    ...
    df = _ensure_cols_from_index(df_with_triggers, names=("subject_id", "hadm_id")).copy()
    ...
    if df["action_cluster_id"].notna().any():
        df.loc[df["action_cluster_id"].notna(), "action_cluster_id"] = \
            df.loc[df["action_cluster_id"].notna(), "action_cluster_id"].astype(int)
        df.loc[df["action_cluster_size"].notna(), "action_cluster_size"] = \
            df.loc[df["action_cluster_size"].notna(), "action_cluster_size"].astype(int)
        df.loc[df["action_cluster_rank"].notna(), "action_cluster_rank"] = \
            df.loc[df["action_cluster_rank"].notna(), "action_cluster_rank"].astype(int)
...
def main(save_dir = None):
    ...
    trigger_ids = pd.to_numeric(s.where(mask_num), errors="coerce").dropna().astype("int64")
    ...
    input_mv_triggers = input_mv_triggers.sort_values(["hadm_id","start_time"] if "subject_id" not in input_mv_triggers.columns else ["subject_id","hadm_id","start_time"])
    input_mv_triggers = compute_action_clusters(input_mv_triggers, window_minutes=20, min_events_in_cluster=1)
