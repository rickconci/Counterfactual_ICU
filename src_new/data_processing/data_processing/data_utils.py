...
def _enforce_pandas_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    if 'hadm_id' in df: df['hadm_id'] = pd.to_numeric(df['hadm_id'], errors='coerce').astype('Int64')
    if 'time_seconds' in df: df['time_seconds'] = pd.to_numeric(df['time_seconds'], errors='coerce').astype('Int64')
    for c in ['ABP MEAN','NBP MEAN','CVP','HR','RESP']:
        if c in df: df[c] = pd.to_numeric(df[c], errors='coerce').astype('float64')
    if 'record_name' in df: df['record_name'] = df['record_name'].astype('string')
