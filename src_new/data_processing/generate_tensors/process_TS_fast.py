...
def pivot_and_pad_physio_optimized(physio_df, name_list, T):
    ...
    pivot_df = physio_df.pivot_table(
        index="rounded_charttime", columns="name", values="value", aggfunc="first"
    ).sort_index()
    ...
    return (
        values_tensor.clone(),
        mask_tensor.clone(),
        abs_time_tensor.clone(),
        rel_time_tensor.clone(),
        length,
    )
...
def process_hadm(hadm_group, name_list, T, cache_dir):
    ...
    (values_tensor, mask_tensor, abs_time_tensor, rel_time_tensor, length) = (
        pivot_and_pad_physio_optimized(physio_sub_df, name_list, T)
    )
...
if __name__ == "__main__":
    ...
    if not np.issubdtype(combined_physio_df["charttime"].dtype, np.datetime64):
        combined_physio_df["charttime"] = pd.to_datetime(
            combined_physio_df["charttime"], errors="coerce"
        )
    combined_physio_df["rounded_charttime"] = combined_physio_df["charttime"].dt.round(
        "3h"
    )

    name_list = (
        combined_physio_df["name"].value_counts().head(top_how_many).index.tolist()
    )
    T = max_T_events
    ...
    process_func = partial(
        process_hadm, name_list=name_list, T=T, cache_dir=str(cache_dir)
    )
