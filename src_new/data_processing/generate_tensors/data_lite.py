...
class MIMICContrastivePairsDatasetLite(Dataset):
    def __init__(
        ...
        self._label_mm = np.memmap(
            os.path.join(label_dir, "labels_scalar.bin"), dtype=np.uint8, mode="r"
        ).reshape(N, 2)
        # 3) padded index matrices (mmap)
        self._cur_mat = np.load(
            os.path.join(label_dir, "current_idx_mat.npy"), mmap_mode="r"
        )
        self._cur_len = np.load(
            os.path.join(label_dir, "current_len.npy"), mmap_mode="r"
        )
        self._nxt_mat = np.load(
            os.path.join(label_dir, "next_idx_mat.npy"), mmap_mode="r"
        )
        ...
        self.phecode_size = pickle.load(
            open(os.path.join(cache_dir, "phecode_size.pkl"), "rb")
        )
    ...
    def filter_valid_samples(self):
        ...
        print(
            f"Filtered {invalid_count} invalid hadm_ids, {len(valid_samples)} remaining"
        )
    ...
    def _fetch_labels(self, hadm_id: int):
        ...
        if row is None:
            ...
            return (
                0,
                0,
                torch.full((self.K,), self.P, dtype=torch.long),
                0,
                torch.full((self.K,), self.P, dtype=torch.long),
                0,
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.test_ds_only:
            ...
            embedding_path = os.path.join(
                self.ds_embeddings_dir, f"embedding_{hadm_id}.pt"
            )
            embedding = torch.load(embedding_path)
            # print(f"embedding: {embedding.shape}")
            mort, readm, cur_pad, cur_len, nxt_pad, nxt_len = self._fetch_labels(
                hadm_id
            )
        ...
        if self.task_mode == "CONTRASTIVE" or self.task_mode == "MULTITASK":
            ...
            ds_emb = (
                torch.load(ds_path) if os.path.exists(ds_path) else torch.zeros(0, 768)
            )
    ...
    def compute_samples(self, cache_file=None):
        ...
        if os.path.exists(cache_file):
            try:
                ...
                print(
                    f"Found {pos_samples} positive samples ({pos_samples/len(self.samples)*100:.2f}%)"
                )
                return
            except Exception as e:
        ...
        for hadm_id in tqdm(self.hadm_ids, desc="Computing sample windows"):
            ...
            while True:
                ...
                if i % stride == 0:
                    ...
                    label_end_time = chunk_end_time + pd.Timedelta(
                        hours=self.label_window
                    )
                    if (
                        deathtime is not None
                        and label_start_time <= deathtime < label_end_time
                    ):
                        label = 1
                    else:
                        label = 0
    ...
    def split_by_subject_id(
        self, df, train_ratio=0.7, val_ratio=0.29, test_ratio=0.01, random_state=42
    ):
        """
        ...
        if df is None:
            raise ValueError(
                "Merged dataframe not loaded. Call load_demographics() first."
            )
        ...
        print(
            f"Split by subject_id: {len(train_subjects)} train, {len(val_subjects)} val, {len(test_subjects)} test subjects"
        )
        print(
            f"Corresponding to: {len(train_hadm_ids)} train, {len(val_hadm_ids)} val, {len(test_hadm_ids)} test admissions"
        )
    ...
    def get_split_hadm_ids(self, split="train"):
        ...
        if split == "train":
            ...
            raise ValueError(
                f"Unknown split: {split}. Must be one of 'train', 'val', 'test'"
            )
