...
class LatentSDE(torchsde.SDEIto):
    ...
    def __init__(self, SDE_latents_dim, mu=None, sigma=None, theta=1.0):
        ...
        # Prior drift.
        self.register_buffer("theta", torch.tensor([[theta]] * SDE_latents_dim).float())
        ...
        self.net = nn.Sequential(
            nn.Linear(2 + SDE_latents_dim, 200),
            nn.Tanh(),
            nn.Linear(200, 200),
            nn.Tanh(),
            nn.Linear(200, SDE_latents_dim)
        )
    ...
    def f(self, t, y):  # Approximate posterior drift.
        ...
        SDE_latents = self.net(torch.cat((torch.sin(t), torch.cos(t), y), dim=-1))
    ...
    def sample_p(self, ts, SDE_samples, eps=None, bm=None):
        eps = torch.randn(SDE_samples, self.SDE_latents_dim).to(self.py0_mean) if eps is None else eps
        y0 = self.py0_mean + eps * self.py0_std
        print('y0 p_sample', y0.shape)
        return sdeint_fn(self, y0, ts, bm=bm, method='srk', dt=args.dt, names={'drift': 'h'})

    def sample_q(self, ts, SDE_samples, eps=None, bm=None):
        eps = torch.randn(SDE_samples, self.SDE_latents_dim).to(self.qy0_mean) if eps is None else eps
        y0 = self.qy0_mean + eps * self.qy0_std
        print('y0 q_sample', y0.shape)
        print('ts q_sample', ts.shape)
...
def main():
    ...
    eps = torch.randn(vis_num_samples, 1).to(device)  # Fix seed for the random draws used in the plots.
    bm = torchsde.BrownianInterval(
        t0=ts_vis[0],
        t1=ts_vis[-1],
        size=(vis_num_samples, 1),
        device=device,
        levy_area_approximation='space-time'
    )  # We need space-time Levy area to use the SRK solver

    # Model.
    model = LatentSDE(SDE_latents_dim = 1).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='segmented_cosine', choices=['segmented_cosine', 'irregular_sine','cv_data'])
    ...
    parser.add_argument('--SDE_samples', type=int, default=512, help='num_samples for training.')
