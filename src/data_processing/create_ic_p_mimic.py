...
class WaveformRecordCache:
    ...
    def discover_patient_records(
        self, patient_id, waveform_base_dir=None, database="mimic3wdb-matched"
    ):
        ...
        if waveform_base_dir:
            ...
            for header_file in numerics_headers:
                try:
                    ...
                    if record is not None:
                        record_info = self._extract_record_info(
                            record, patient_path, record_name
                        )
                        if record_info:
            ...
            try:
                ...
                for hea_file in numerics_files:
                    ...
                    try:
                        record = self._get_cached_header(
                            record_name, full_pn_dir, patient_path
                        )
                        if record is not None:
                            record_info = self._extract_record_info(
                                record, patient_path, record_name
        ...
        print(
            f"    Discovered {len(records)} waveform records for patient {patient_id}"
        )
        return records
    ...
    def find_records_for_timerange(self, patient_id, start_time, end_time):
        ...
        for record_info in records:
            ...
            if (
                record_info["start_time"] <= end_time
                and record_info["end_time"] >= start_time
            ):
    ...
    def load_record_data(
        self, record_info, database="mimic3wdb-matched", max_retries=5
    ):
        """Load actual waveform data for a record (cached), with retries."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.record_data[cache_key] = {"record": record, "info": record_info}

                # This print can be noisy, let's keep it minimal or debug-only
                # print(
                #    f"      Loaded record {record_info['record_name']} "
                ...
                if "timed out" in str(e) or "new connection" in str(e):
                    if attempt < max_retries - 1:
                        wait_time = (2**attempt) + random.uniform(
                            0, 1
                        )  # Exponential backoff with jitter
                        debug_print(
                            f"      Connection error for {record_info['record_name']}. Retrying in {wait_time:.2f}s... (Attempt {attempt + 1}/{max_retries})"
                        )
                        time.sleep(wait_time)
                    else:
                        print(
                            f"      Error loading record {record_info['record_name']} after {max_retries} attempts: {e}"
                        )
                        return None
                else:
                    # It's a different, non-retryable error, so fail immediately
                    print(
                        f"      Non-retryable error loading record {record_info['record_name']}: {e}"
                    )
                    return None
...
def find_signal_index(record_signal_names, target_signal):
    ...
    for i, record_signal in enumerate(record_signal_names):
        for possible_name in possible_names:
            if possible_name.lower().replace(" ", "").replace(
                "_", ""
            ) in record_signal.lower().replace(" ", "").replace("_", ""):
                return i
...
def extract_signals_from_record(record_data, start_time, end_time, target_signals):
    ...
    end_offset = min(
        record_info["duration_seconds"], (end_time - record_start).total_seconds()
    )
...
def extract_ic_and_create_predictions_with_clean_separation(
    hadm_id,
    traj_num,
    t0_timestamp,
    record_data,
    ...
    if not signals_data:
        print(
            f"      ✗ No waveform data found within ±{window_seconds}s of t₀ - REJECTING trajectory"
        )
        return None

    # Check that we have the required prediction signals within ±30s
    missing_prediction_signals = [
        sig for sig in prediction_signals if sig not in signals_data
    ]
    ...
    for signal_name in required_ic_signals + optional_ic_signals:
        ...
        print(
            f"      ✓ IC {signal_name}: {closest_value:.1f} at {time_diff_seconds:.1f}s from t₀"
        )
    ...
    if ic_mask.get("HR", 0) > 0 and ic_values.get("HR", 0) > 0:
        ...
        if (
            ic_mask.get("CO", 0) > 0
            and ic_mask.get("SV", 0) == 0
            and ic_values.get("CO", 0) > 0
        ):
        ...
        # If we have SV but missing CO: calculate CO = SV * HR / 1000
            ...
            ic_mask.get("SV", 0) > 0
            and ic_mask.get("CO", 0) == 0
            and ic_values.get("SV", 0) > 0
        ):
    ...
    # Calculate R_TPR
    if (
        ic_mask.get("ABP Mean", 0) > 0
        and ic_mask.get("CVP", 0) > 0
        and ic_mask.get("CO", 0) > 0
        and ic_values.get("CO", 0) > 0
    ):
    ...
    print(
        f"      ✓ Prediction duration: {actual_prediction_duration} minutes from reference timepoint"
    )
    ...
    for signal_name in prediction_signals:
        ...
        for minute_idx in range(total_minutes):
            ...
            points_in_minute = np.sum(
                (signal_times >= minute_start) & (signal_times < minute_end)
            )
        ...
        if minutes_failed:
            print(
                f"      ✗ REJECTING trajectory {traj_num}: {signal_name} fails strict per-minute requirement"
            )
            print(
                f"        {len(minutes_failed)} out of {total_minutes} minutes have insufficient data"
            )
            ...
            print(
                f"        STRICT POLICY: ALL {total_minutes} minutes must have >= {min_points_per_minute} points"
            )
            return None
        else:
    ...
    n_prediction_points = (
        int((actual_prediction_duration * 60) / target_interval_seconds) - 1
    )
    ...
    for signal_name in prediction_signals:
        ...
        try:
            ...
            if len(unique_times) < 2:
                print(
                    f"      ✗ {signal_name}: Less than 2 unique time points after deduplication"
                )
                return None
            ...
            if np.any(np.isnan(interpolated_values)):
                ...
                print(
                    f"      ✗ {signal_name}: {n_nans}/{len(interpolated_values)} NaN values after interpolation"
                )
                return None
    ...
    mask_array = np.ones(
        (n_prediction_points, n_signals), dtype=np.float32
    )  # All 1s since no NaNs
    ...
    time_seconds_from_prediction_start = np.array(
        [(i + 1) * target_interval_seconds for i in range(n_prediction_points)]
    )
...
def save_full_trajectory_data(
    ...
    if not signals_data:
        print(
            f"      [Full Traj] No signals data found for record {record_info['record_name']}"
        )
        return
    ...
    interpolated_signals = interpolate_signals_to_grid(
        signals_data, target_times_seconds
    )

    if not interpolated_signals:
        print(
            f"      [Full Traj] Interpolation failed for record {record_info['record_name']}"
        )
        return
    ...
    # Save to parquet
    output_file = (
        Path(full_trajectory_dir)
        / f"full_waveform_{int(hadm_id)}_{record_info['record_name']}.parquet"
    )
    df.to_parquet(output_file, index=False)
    print(
        f"      [Full Traj] Saved full waveform record to {output_file} with {len(t0s_in_record)} t0s."
    )


def discover_records_for_patient_worker(args):
    """
    Worker process to discover all waveform records for a single patient.
    """
    hadm_id, patient_id, database = args
    # Each worker needs its own cache instance to avoid sharing/pickling issues.
    cache = WaveformRecordCache()
    records = cache.discover_patient_records(patient_id, database=database)
    return hadm_id, records


def download_record_worker(
    args,
):
    """
    Worker process to download, process, and save a single waveform record.
    """
    (
        hadm_id,
        icu_admission_time,
    ...
    if record_data:
        save_full_trajectory_data(
            ...
            all_patient_t0s=[],  # No t0s in download-only mode
...
def process_patient_integrated_clean_separation(
    hadm_id,
    patient_id,
    ...
    patient_records = waveform_cache.discover_patient_records(
        patient_id, database=database
    )
    ...
    if save_full_trajectory and full_trajectory_dir:
        ...
        for record_info in patient_records:
            ...
            if not output_file.exists():
                print(
                    f"    [Full Traj] Saving new record {record_info['record_name']}."
                )
                record_data = waveform_cache.load_record_data(record_info, database)
                if record_data:
                    save_full_trajectory_data(
                ...
                debug_print(
                    f"    [Full Traj] Record {record_info['record_name']} already exists. Skipping."
                )

    # Step 3: Generate IC and Prediction Targets (t0-centric)
    print(
        f"  Generating ICs and Prediction Targets for {len(patient_trajectories)} t0-based trajectories..."
    )
    patient_ics = []
    patient_predictions = []
    processed_count = 0
    ...
    for traj in patient_trajectories:
        ...
        try:
            t0_timestamp = calculate_t0_timestamp(
                traj, icu_admission_time, interval_minutes
            )
            ...
            if not overlapping_records:
                print(
                    f"    ✗ Skipping t0-trajectory {traj_num} - no records overlap with window"
                )
                skipped_count += 1
                continue
            ...
            if len(overlapping_records) > 1:
                print(
                    f"    ✗ Skipping t0-trajectory {traj_num} - window spans {len(overlapping_records)} records"
                )
                skipped_count += 1
                continue
    ...
    print(
        f"  Created {len(patient_ics)} IC tensors and {len(patient_predictions)} prediction targets"
    )
...
def process_patient_and_save(
    ...
    for ic_result in patient_ics:
        ...
        ic_file = (
            Path(ic_cache_dir) / f"ic_tensor_{int(hadm_id)}_traj_{traj_num:03d}.pt"
        )

        # Granular skipping: only write file if it doesn't exist
        if skip_existing and ic_file.exists():
            debug_print(
                f"  [Worker PID: {os.getpid()}] Skipping existing IC tensor for traj {traj_num}"
            )
            continue
    ...
    for prediction in patient_predictions:
        ...
        if skip_existing and pred_file.exists():
            debug_print(
                f"  [Worker PID: {os.getpid()}] Skipping existing prediction tensor for traj {traj_num}"
            )
            continue

        debug_print(
            f"  [Worker PID: {os.getpid()}] Saving prediction tensor for traj {traj_num}"
        )
    ...
    debug_print(
        f"[Worker PID: {os.getpid()}] Finished saving for patient {hadm_id}. Returning metadata."
    )
...
def create_integrated_waveform_tensors(
    ...
    """
    Main function to create integrated waveform tensors.

    Two modes of operation:
    1. Standard Mode (save_full_trajectory=False):
       - Processes patient trajectories based on `trajectory_metadata_path`.
       - Generates initial condition (IC) tensors and prediction target tensors.
       - Caches these tensors to `ic_cache_dir` and `prediction_cache_dir`.

    2. Download-Only Mode (save_full_trajectory=True):
       - Ignores trajectory metadata and focuses on downloading raw waveform data.
       - Takes a list of patients from `relevant_patient_ids_path`.
       - Discovers and saves all available waveform records for each patient into `full_trajectory_dir`.
       - Does NOT create any IC or prediction tensors.
    """
    debug_print(
        "--- Starting Integrated Waveform Tensor Creation with IC-Anchored Validation ---"
    )
    ...
    if save_full_trajectory:
        print("--- Running in Waveform Download-Only Mode ---")
        ...
        # Step 1: Discover all records for all patients in parallel
        print("Phase 1: Discovering all waveform records in parallel...")
        discovery_tasks = [
            (hadm_id, patient_id, database)
            for hadm_id, patient_id in patient_mapping.items()
        ]
        all_discovered_records = []
        with concurrent.futures.ProcessPoolExecutor(max_workers=n_workers) as executor:
            results = list(
                tqdm(
                    executor.map(discover_records_for_patient_worker, discovery_tasks),
                    total=len(discovery_tasks),
                    desc="Discovering Patient Records",
                )
            )
            for hadm_id, records in results:
                all_discovered_records.append((hadm_id, records))

        # Step 2: Aggregate tasks and check for existing files
        print("Phase 2: Aggregating records and preparing download tasks...")
        download_tasks = []
        for hadm_id, records in all_discovered_records:
            if hadm_id not in icu_admission_map:
                continue
            icu_admission_time = icu_admission_map[hadm_id]

            for record_info in records:
                # Check if the output file already exists before adding to the task list
                output_file = (
                    Path(full_trajectory_dir)
                    / f"full_waveform_{int(hadm_id)}_{record_info['record_name']}.parquet"
                )
                if skip_existing and output_file.exists():
                    continue  # Don't even add it to the queue
        ...
        # Step 3: Download all records in parallel
        print(
            f"Phase 3: Found {len(download_tasks)} new records to download. Starting parallel download..."
        )
        with concurrent.futures.ProcessPoolExecutor(max_workers=n_workers) as executor:
            list(
                tqdm(
                    executor.map(download_record_worker, download_tasks),
                    total=len(download_tasks),
                    desc="Downloading Waveform Records",
                )
            )
    ...
    # --- Standard Mode: IC and Prediction Tensor Creation ---
    debug_print("--- Running in Standard Mode (IC & Prediction Tensor Creation) ---")
    ...
    for hadm_id in list(patient_mapping.keys()):  # [:200]:  # TODO: remove this limit
        ...
        if skip_existing:
            ...
            if hadm_id not in all_trajectories:
                debug_print(
                    f"Skipping patient {hadm_id} - no trajectories found in metadata."
                )
                skipped_patients += 1
                continue
            ...
            for traj in all_trajectories[hadm_id]:
                ...
                ic_file = (
                    Path(ic_cache_dir)
                    / f"ic_tensor_{int(hadm_id)}_traj_{traj_num:03d}.pt"
                )
                pred_file = (
                    Path(prediction_cache_dir)
                    / f"prediction_target_{int(hadm_id)}_traj_{traj_num:03d}.pt"
            ...
            if all_files_exist:
                debug_print(
                    f"Skipping patient {hadm_id} - all IC/prediction files exist (fast path)."
                )
                skipped_patients += 1
                continue
        ...
        if not patient_trajectories:
            debug_print(f"No trajectories for HADM_ID {hadm_id}, skipping.")
            continue
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_workers) as executor:
        future_to_hadm = {
            executor.submit(process_patient_and_save, *task): task[0] for task in tasks
        }

        for future in tqdm(
            concurrent.futures.as_completed(future_to_hadm),
            total=len(tasks),
    ...
    print(
        f"  Validation method: IC-anchored interpolation with {min_points_per_minute} points/min"
    )
...
if __name__ == "__main__":
    ...
    trajectory_metadata_path = (
        ...
        / "processed_data"
        / "med_tensors"
        / "trajectory_metadata.pkl"
    )
    relevant_patient_ids_path = (
        DEFAULT_OUTPUT_DIR / "processed_data" / "relevant_patient_ids.csv"
    )
    ...
    # To run in download-only mode, set save_full_trajectory=True
    # This will ignore trajectory metadata and just download all waveforms
    # for patients in relevant_patient_ids.csv
