...
def main(args):
    ...
    if args.dataset_type == 'synthetic':
        ...
        cv_data_module_IID = CVDataModule_IID(train_val_data=train_val_data, batch_size=args.batch_size, num_workers=0,
                                              debug=args.debug)  # <<< Pass debug flag >>>
        ...
        cv_data_module_OOD = CVDataModule_OOD(OOD_test_data=test_data, batch_size=args.batch_size, num_workers=0,
                                              debug=args.debug)  # <<< Pass debug flag >>>
...
if __name__ == '__main__':
    ...
    # Data specific args
    parser.add_argument('--dataset_type', type=str, default='synthetic', choices=['synthetic', 'mimic'],
                        help='Which dataset to use.')
    parser.add_argument('--data_root', type=str, default='../data/mimic_3_data/processed_data',
                        help='Root directory for MIMIC preprocessed data.')
    parser.add_argument('--icu_stays_path', type=str, default='data/input_data/icustays.csv',
                        help='Path to icustays.csv file.')
    parser.add_argument('--static_hidden_dim', type=int, default=16,
    ...
    parser.add_argument('--non_confounded_effect', type=bool, default=False,
    ...
    parser.add_argument('--include_all_inputs', type=bool, default=True,
    ...
    parser.add_argument('--confounder_type', type=str, default='partial_hard',
                        choices=['visible', 'partial', 'partial_hard', 'invisible'],
                        help='the type of confounding present')
    parser.add_argument('--use_encoder', type=str, default='none', choices=['full', 'partial', 'none', 'raindrop'],
    ...
    parser.add_argument('--include_time', type=bool, default=True,
                        help='Whether to include encoded time in the SDE NN inputs)')

    parser.add_argument('--prior_tx_sigma', type=float, default=0.05,
                        help='prior_tx_sigma defines our assumed prior noise of the stochastic control ')
    parser.add_argument('--self_reverting_prior_control', type=bool, default=False,
                        help='Whether the control has a self reverting prior to it with a functional prior')
    parser.add_argument('--KL_weighting_SDE', type=float, default=0.001,
                        help='Defines the weighting to the KL loss for the SDE')

    parser.add_argument('--use_2_5std_encoder_minmax', type=bool, default=False,
    ...
    parser.add_argument('--prior_tx_mu', type=float, default=0.01,
    ...
    parser.add_argument('--SDE_control_weighting', type=float, default=1,
                        help='how much to scale the output of the SDE NN')

    # Model specific args
    parser.add_argument('--start_dec_at_treatment', type=bool, default=True,
                        help='Whether to encode the data until treatment and the decode or decode from the beginning!)')
    parser.add_argument('--variational_encoder', type=bool, default=False,
    ...
    parser.add_argument('--encoder_w_time', type=bool, default=False,
                        help='Whether encoder includes time in its inputs)')
    parser.add_argument('--encoder_reverse_time', type=bool, default=False,
                        help='Whether encoder runs with inputs backwards in time)')
    parser.add_argument('--variational_sampling', type=bool, default=False,
                        help='If NOT using encoder, to learn a variational q distribution for the unobserved dims)')

    parser.add_argument('--final_activation', type=str, default='none', choices=['relu', 'none'],
                        help='Which nonlinearity to add as a final layer to the NN!')
    parser.add_argument('--normalise_for_SDENN', type=bool, default=True,
    ...
    parser.add_argument('--SDE_input_state', type=str, default='full', choices=['full', 'partial'],
    ...
    parser.add_argument('--accelerator', type=str, default='auto', choices=['gpu', 'mps', 'cpu', 'auto'],
