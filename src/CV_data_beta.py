...
from utils_beta import CV_params_prior_mu, CV_params_prior_sigma, CV_params
...
class PhysiologicalSDE_batched(torchsde.SDEIto):
    def __init__(self, sigma, sigma_tx, params, confounder_type,non_confounded_effect, debug=False):
        ...
        self.debug = debug
        if self.debug: print(f"[DEBUG] PhysiologicalSDE_batched __init__: Initialized with confounder_type: {self.confounder_type}, non_confounded_effect: {self.non_confounded_effect}")
    ...
    def rate_of_change_of_flow(self, t, volumes, durations, width_factor=3):
        # Ensure all inputs are tensors and computations are done in float32
        width_factor = torch.tensor(width_factor, dtype=torch.float32)

        # Define mean and sigma for each entry in the batch
        means = durations / 2
        sigmas = width_factor * durations / 10

        # Calculate A for each volume and duration in the batch
        A = volumes / (torch.sqrt(torch.tensor(np.pi)) * sigmas * erf((durations-means)/sigmas))

        # Calculate derivative for each t in batch
    ...
    def create_treatment_effect(self):
        ...
        if self.confounder_type == 'visible':
            ...
            A_ = self.v_fun(init_sv_transform, scale = 0.1)
        elif self.confounder_type == 'partial_hard':
            init_sv_transform  = 0.5+(self.params["confounding_sv"]-0.19)/0.1 
            A_ = self.v_fun(init_sv_transform, scale = 0.05)
    ...
    def v_fun(self, x, scale):
        ...
        square_term = (5 - x) ** 2
        # <<< Conditional print for v_fun output if shapes are unexpected or values seem off >>>
        # if self.debug: print(f"[DEBUG] v_fun: input x: {x}, scale: {scale}, output: {scale * (cos_term * square_term) ** 2}")
        return scale * (cos_term * square_term) ** 2

    def f(self, t, y):
        if self.debug and t.item() % 10 == 0: # Print every 10 time steps to avoid too much spam
            print(f"[DEBUG] PhysiologicalSDE_batched.f: t={t.item()}, y_shape={y.shape}")
        
        if abs(t.item() - 40.0) < 0.001:
            if self.debug: print(f"[DEBUG] PhysiologicalSDE_batched.f: t={t.item()} (treatment decision point). Updating params and creating treatment effect.")
            ##print(t)
            #print('saving params')
        ...
        print('time, i_ext pre, A_, i_ext_post, pa, sv', t.item(),i_ext[0].item(), A_[0], i_ext_tx_effect[0].item(), p_a[0].item(), sv[0].item())   
        ...
        dsv_dt = i_ext_tx_effect * 0.001

        if self.params["treatment"] and (t >= self.params["t_treatment"]):
            #note that the 
            time_since_treatment = t - self.params["t_treatment"]
            diext_dt = self.rate_of_change_of_flow(t = time_since_treatment, volumes =y[:, 5], durations =y[:, 6])
            #diext_dt = torch.relu(i_ext + diext_dt) - i_ext 
        else:
            #diext_dt = torch.full((batch_size, 1), 1)
            diext_dt = torch.zeros_like(dpa_dt)

        print('diext_dt', diext_dt.shape, diext_dt[:])        
        diff_res = torch.concat([dpa_dt, dpv_dt, ds_dt, dsv_dt, diext_dt,torch.zeros([y.shape[0], 2])], dim=-1)
    ...
    def g(self, t, y):
        if self.debug and t.item() % 10 == 0: # Print every 10 time steps
            print(f"[DEBUG] PhysiologicalSDE_batched.g: t={t.item()}, y_shape={y.shape}")
...
def init_random_state(fixed_tx = True, debug=False):
    if debug: print(f"[DEBUG] init_random_state: fixed_tx={fixed_tx}")
    max_ves = 64.0 - 10.0
    min_ves = 36.0 + 10.0
    ...
    init_state = np.array([init_pa, init_pv, init_s, init_sv, init_i_ext, vol, duration])

    if debug: print(f"[DEBUG] init_random_state: generated init_state[:3]={init_state[:3]}")
    return init_state


def create_cv_data(fixed_tx, include_all_inputs, N,gamma,noise_std, sigma_tx, r_tpr_mod, confounder_type, non_confounded_effect, t_span, t_treatment, t_cutoff, seed, post_treatment_dims, pre_treatment_dims, normalize = False, debug=False):
    if debug: print(f"[DEBUG] create_cv_data: Starting data generation. N={N}, gamma={gamma}, confounder_type={confounder_type}, fixed_tx={fixed_tx}")
    ...
    init_state_list = []
    for i in range(N):
        init_state_list.append(init_random_state(fixed_tx=fixed_tx))
    ...
    sde = PhysiologicalSDE_batched(sigma = noise_std, sigma_tx=sigma_tx, confounder_type=confounder_type , non_confounded_effect = non_confounded_effect, params=params_treatment, debug=debug)
    #print(f"Running sdeint for treated case. N={len(init_state_list)}")
    if debug: print(f"[DEBUG] create_cv_data: Running sdeint for TREATED case. Num initial states: {len(init_state_list)}")
    with torch.no_grad():
        y1 = torchsde.sdeint(sde, init_state_tensor, t_tensor, method='euler',dt=0.05).squeeze(1)
    if debug: print(f"[DEBUG] create_cv_data: sdeint for TREATED case completed. Output shape: {y1.shape}")

    #print('creating untreated')
    #created untreated from the same initial random satates
    sde = PhysiologicalSDE_batched(sigma = noise_std, sigma_tx=sigma_tx, confounder_type=confounder_type , non_confounded_effect = non_confounded_effect, params=params_notreatment, debug=debug)
    #print(f"Running sdeint for untreated case. N={len(init_state_list)}")
    if debug: print(f"[DEBUG] create_cv_data: Running sdeint for UNTREATED case. Num initial states: {len(init_state_list)}")
    with torch.no_grad():
        y0 = torchsde.sdeint(sde, init_state_tensor, t_tensor, method='euler', dt=0.05).squeeze(1)
    if debug: print(f"[DEBUG] create_cv_data: sdeint for UNTREATED case completed. Output shape: {y0.shape}")

    print('Y0 Y1', y0.shape, y1.shape)
    X = y0[t_cutoff, :, :]
    init_state = y0[t_cutoff, :, :] #the confounder is not based on the 'initial' values but rather the ones just before treatment 
    ...
    elif confounder_type == 'partial_hard':
        scaled_sv = scale_numbers(x=init_state[:, 3], original_min=0.83, original_max=0.9, target_min=0, target_max=1)
        p  = 1- torch.sigmoid(gamma*scaled_sv) 
    ...
    #all_trajectories = torch.cat([y0, y1], dim = 1)
    y0 = y0[:, :, :4].permute(1, 0, 2)  # drop the dim used to create I-external and permute to [batch, seq_len, dim]
    y1 = y1[:, :, :4].permute(1, 0, 2)
    T_expanded = T[:, None, None]
    #print('Y0, Y1, T_expanded', y0.shape, y1.shape, T_expanded.shape)
    print('Y0 Y1 permuted', y0.shape, y1.shape)
    ...
    Y_fact = y0 * (1-T_expanded)+ y1 *T_expanded
    ...
    Y_cf = y0 * T_expanded + y1 * (1-T_expanded)
    ...
    if debug: print(f"[DEBUG] create_cv_data: Data generation finished.")
...
def create_load_save_data(dataset_params, data_path):
    debug = dataset_params.get('debug', False) # <<< Get debug flag >>>
    if debug: print(f"[DEBUG] create_load_save_data: Params received: {dataset_params}, Data path: {data_path}")

    # Construct a unique filename based on dataset_params
    filename_parts = [
        f"N={dataset_params['N']}",
        f"seed={dataset_params['seed']}",
        f"gamma={dataset_params['gamma']}",
        f"noise_std={dataset_params['noise_std']}",
        f"sigma_tx={dataset_params['sigma_tx']}",
        f"r_tpr_mod={dataset_params['r_tpr_mod']}",
        f"conf_type={dataset_params['confounder_type']}",
        f"fixed_tx={'T' if dataset_params['fixed_tx'] else 'F'}",
        f"all_inputs={'T' if dataset_params['include_all_inputs'] else 'F'}"
    ]
    filename_str = "_".join(filename_parts)
    filename = os.path.join(data_path, filename_str + ".pt")

    if os.path.exists(filename):
        if debug: print(f"[DEBUG] create_load_save_data: Loading data from {filename}")
        data = torch.load(filename)
        if debug: print(f"[DEBUG] create_load_save_data: Data loaded successfully.")
    else:
        if debug: print(f"[DEBUG] create_load_save_data: Data file not found. Creating data...")
        # Generate data which returns a tuple
        generated_data_tuple = create_cv_data(
            fixed_tx=dataset_params['fixed_tx'], 
            include_all_inputs=dataset_params['include_all_inputs'],
            N=dataset_params['N'],
        ...
        # Pack the tuple into a dictionary with correct keys
        data_keys = ['X', 'X_static', 'T', 'Y_fact', 'Y_cf', 'p', 'init_state', 't_X', 't_Y', 'expert_ODE_size', 't_full', 'full_fact_traj', 'full_CF_traj', 'sde']
        data = dict(zip(data_keys, generated_data_tuple))

        os.makedirs(data_path, exist_ok=True)
        torch.save(data, filename)
        if debug: print(f"[DEBUG] create_load_save_data: Data created and saved to {filename}")
    return data
...
class CVDataModule_IID(L.LightningDataModule):
    def __init__(self, train_val_data,  batch_size=32, num_workers =1, debug=False): # <<< Add debug flag >>>
        super().__init__()
        ...
        if self.debug: print(f"[DEBUG] CVDataModule_IID __init__: batch_size={batch_size}, num_workers={num_workers}")

    def setup(self, stage=None):
        if self.debug: print(f"[DEBUG] CVDataModule_IID setup: stage={stage}")
        # Load the dataset
        ...
        if self.debug:
            print(f"[DEBUG] CVDataModule_IID setup: train_dataset size: {len(self.train)}")
            print(f"[DEBUG] CVDataModule_IID setup: val_dataset size: {len(self.val)}")
            print(f"[DEBUG] CVDataModule_IID setup: test_dataset size: {len(self.in_dist_test)}")
            print(f"[DEBUG] CVDataModule_IID setup: encoder_input_dim: {self.encoder_input_dim}, expert_latent_dim: {self.expert_latent_dim}")

    def train_dataloader(self):
        if self.debug: print(f"[DEBUG] CVDataModule_IID train_dataloader: Creating train DataLoader.")
        return DataLoader(
            self.train,
    ...
    def val_dataloader(self):
        if self.debug: print(f"[DEBUG] CVDataModule_IID val_dataloader: Creating val DataLoader.")
        return DataLoader(
            self.val,
            batch_size=self.batch_size,
            shuffle=True,
    ...
    def test_dataloader(self):
        if self.debug: print(f"[DEBUG] CVDataModule_IID test_dataloader: Creating test DataLoader.")
        return DataLoader(
            self.in_dist_test,
            batch_size=self.batch_size,
            shuffle=False,
...
class CVDataModule_OOD(L.LightningDataModule):
    def __init__(self, OOD_test_data,  batch_size=32, num_workers =1, debug=False): # <<< Add debug flag >>>
        super().__init__()
        ...
        self.debug = debug # <<< Store debug flag >>>
        if self.debug: print(f"[DEBUG] CVDataModule_OOD __init__: batch_size={batch_size}, num_workers={num_workers}")

    def setup(self, stage=None):
        if self.debug: print(f"[DEBUG] CVDataModule_OOD setup: stage={stage}")
        self.OOD_test_dataset = CVDataset_loaded(self.OOD_test_data)
        if self.debug: print(f"[DEBUG] CVDataModule_OOD setup: OOD_test_dataset size: {len(self.OOD_test_dataset)}")

    def test_dataloader(self):
        if self.debug: print(f"[DEBUG] CVDataModule_OOD test_dataloader: Creating OOD test DataLoader.")
        return DataLoader(
