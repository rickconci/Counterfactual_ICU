import wandb
from utils_beta import interpolate_colors
import numpy as np
...
def plot_factuals_counterfactuals(X, Y, Y_cf, Y_hat, Y_hat_cf, num_batches, chart_type, train_dir, global_step, log_wandb):
    ...
    for batch in range(num_batches):
        for i in range(Y.shape[2]): 
            ...
            mean_Y_hat = Y_hat[batch, :, :, i].mean(0).cpu().numpy()
            fig.add_trace(
                go.Scatter(x=time_y, y=mean_Y_hat, mode='lines', line=dict(color=colors['Predicted'], width=2), name=f'Batch {batch} Mean Predicted_{i}', showlegend=False),
                row=1, col=1
            )
            for sample in range(Y_hat.shape[1]):
                fig.add_trace(
                    go.Scatter(x=time_y, y=Y_hat[batch, sample, :, i].cpu().numpy(), mode='lines', line=dict(color=colors['Predicted']),opacity=0.1, name=f'Batch {batch} Sample {sample} Predicted_{i}', showlegend=False),
                    row=1, col=1
                )
            fig.add_trace(
                go.Scatter(x=time_x, y=X[batch, :, i].cpu().numpy(), mode='lines', line=dict(color=colors['Input']), name=f'Batch {batch} Input_{i}', showlegend=False),
            ...
            mean_Y_hat_cf = Y_hat_cf[batch, :, :, i].mean(0).cpu().numpy()
            fig.add_trace(
                go.Scatter(x=time_y, y=mean_Y_hat_cf, mode='lines', line=dict(color=colors['Predicted Cf'], width=2), name=f'Batch {batch} Mean Predicted Cf_{i}', showlegend=False),
                row=1, col=2
            )
            for sample in range(Y_hat_cf.shape[1]):
                fig.add_trace(
                    go.Scatter(x=time_y, y=Y_hat_cf[batch, sample, :, i].cpu().numpy(), mode='lines', line=dict(color=colors['Predicted Cf']),opacity=0.1, name=f'Batch {batch} Sample {sample} Predicted Cf_{i}', showlegend=False),
                    row=1, col=2
                )

    fig.update_layout(
        title=f"{chart_type} Longitudinal Trajectories",
        height=600,
        width=1200,
        xaxis_title="Time",
        yaxis_title="Values",
    ...
    plot_filename = os.path.join(train_dir, f'Predictions_global_step_{global_step}.png')
    fig.write_image(plot_filename, engine="kaleido")  # Ensure Kaleido is installed for static image export
    print(f'Saved figure at: {plot_filename}')

    if log_wandb:
        wandb.log({f"Factual and Counterfactual Predictions Step {global_step}": wandb.Image(plot_filename)})
...
def plot_grouped_mse(Y, Y_hat, Y_cf, Y_hat_cf, chart_type, train_dir, global_step, log_wandb):
    ...
    for batch in range(Y.shape[0]):
        mse_factual = torch.mean((Y[batch] - Y_hat[batch]) ** 2)  
        mse_cf = torch.mean((Y_cf[batch] - Y_hat_cf[batch]) ** 2)  
    ...
    batches = np.arange(len(mse_array_factual))  
    factual_bars = go.Bar(
        name='Factual MSE',
        x=batches,
        y=mse_array_factual,
...
def plot_SDENN_output(SDE_output, train_dir, global_step, log_wandb):
    # Ensure the 'train_dir' exists, which includes the 'figures' subdirectory
    if not os.path.exists(train_dir):
        os.makedirs(train_dir)

    data_np = SDE_output.cpu().numpy().mean(1) 
    
    batch_dim, num_time_steps, num_dims = data_np.shape
    
    fig = make_subplots(rows=1, cols=num_dims, subplot_titles=[f'Dimension {i+1}' for i in range(num_dims)])
    
    for i in range(batch_dim):
        for j in range(num_dims):
            fig.add_trace(
                go.Scatter(x=np.arange(num_time_steps), y=data_np[i, :, j],
                           mode='lines', name=f'Batch {i+1} Dimension {j+1}'),
                row=1, col=j+1
            )
    
    fig.update_layout(
        title='SDE Model Output Over Time by Dimension',
        xaxis_title='Time Step',
        yaxis_title='Output Value',
        legend_title='Batch and Dimension'
    )
    ...
    if log_wandb:
        wandb.log({f"SDE_Output_Over_Time_{global_step}": wandb.Image(plot_filename)})
...
def plot_trajectories_normal(ts, zs, ys, plotting_config, img_path='trajectory.png', log_to_wandb=False):
    ...
    with torch.no_grad():
        ...
        vis_idx = np.random.permutation(zs.shape[0])[:num_samples]  
        ...
        if log_to_wandb:
            wandb.log({"Trajectories": wandb.Image(fig)})  #
...
def plot_factual_predicted_trajectories(X, Y, Y_hat, chart_type="val"):
    ...
    for i in range(max(X.shape[1], Y.shape[1])):  
        if i < Y_hat.shape[2]:  
...
def plot_counterfactual_predicted_trajectories(X, Y_cf, Y_hat_cf, chart_type="val"):
    ...
    for i in range(max(X.shape[1], Y_cf.shape[1])):  
        if i < Y_hat_cf.shape[2]:  
