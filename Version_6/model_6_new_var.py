...
def main():
    ...
    mu_latents = np.concatenate((first_pa_pv, np.array([0.2, 0.95])))
    sigma_latents = np.array([0.001, 0.001, 0.02, 0.02])
    ...
    for global_step in tqdm.tqdm(range(args.train_iters)):
        ...
        #print('zs edited', zs_to_compare.shape)
        #print('ys', ys.shape)
        ...
        loss = -logpy.mean() + 0.01* kl * kl_scheduler.val
        loss = loss.squeeze() 
        #print(loss)
        ...
        logpy_val = logpy.mean()  # This ensures logpy_val is a scalar
        #print(f"Reduced logpy_val: {logpy_val}")
        logpy_metric.step(logpy_val)  # Pass the scalar logpy_val
        
        kl_val = ensure_scalar(kl_metric.val)
        kl_metric.step(kl_val)

        loss_val = ensure_scalar(loss_metric.val)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pause_iters', type=int, default=3, help='Number of iterations before pausing.')
